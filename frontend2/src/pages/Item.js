...
export default function Item() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/pelucias/${id}`)
      .then(res => {
  ...
  function deletarProduto() {
    ...
    fetch(`http://localhost:5000/pelucias/${produto.id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${usuarioLogado.id}`
      }
    ...
    <div className="pagina-inicial" style={{ maxWidth: 600, margin: '40px auto' }}>
        ...
        <button
          ...
          onClick={() => {
            fetch('http://localhost:5000/colecao', {
              ...
              body: JSON.stringify({ pelucia_id: produto.id })
            })
              .then(res => res.json())
              .then(data => {
                alert(data.mensagem || data.erro);
      ...
      {noCarrinho && (
        <button
          className="botao-perfil"
          onClick={() => {
            fetch(`http://localhost:5000/colecao/${produto.id}`, {
              method: 'DELETE',
              headers: {
                Authorization: `Bearer ${usuarioLogado.id}`
              }
